...
-(void)requestBindingPayAccount:(NSString *)zfbzh wxzh:(NSString *)wxzh success:(void (^)(void))successBlock failure:(HTTPTaskFailureBlock)failureBlock {
    NSMutableDictionary *param = @{ @"api_token":APP_USER.userInfo.api_token}.mutableCopy;
    if (zfbzh) [param addEntriesFromDictionary:@{@"zfbzh":zfbzh}];
    if (wxzh) [param addEntriesFromDictionary:@{@"wxzh":wxzh}];
    [[GJHttpNetworkingManager sharedInstance] requestFormTypePostWithPathUrl:Mine_PayAccountBinding andParaDic:param andSucceedCallback:^(NSURLResponse *urlResponse, id response) {
...
- (void)uploadPortraitFile:(UIImage *)image success:(void (^)(NSString *))successBlock failure:(HTTPTaskFailureBlock)failureBlock {
    if (!image) return ;
    NSData *scaleImgData = [UIImage compressImage:image];
