...
- (void)setCashDetail:(NSString *)cashDetail type:(NSInteger)type {
    _lShopLB.text = @"收款明细";
    _rShopLB.text = cashDetail;
    _shopLB.text = @"贵州全智易联科技有限公司";
    _topLB.text = @"付款金额";
    _tfLeftLB.text = @"¥";
    if (type) {
        _rShopLB.textColor = APP_CONFIG.appMainColor;
    }else {
        _rShopLB.textColor = APP_CONFIG.partnerMainColor;
    }
}
